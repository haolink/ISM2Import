...
namespace ISM2Convert
{
    public class ISMModel
    {
                ...
                if (sd.SectionType == 97)
                {
                    ...
                    for (i = 0; i < matTotal; i++)
                    {
                        ...
                        if (matSubTotal > 0)
                        {
                            ...
                            if(Regex.Match(mat.DiffuseTexture, @"^file(\d+)\.dds$", RegexOptions.CultureInvariant | RegexOptions.IgnoreCase).Success)
                            {
        ...
        private void ImportTriangles()
        {
            ...
            for (i = 0; i < triDataTotal; i++)
            {
                ...
                if(sectionType == 69)
                {
                    ...
                    ushort triType = (ushort)sp.ReadU32();
                    sp.BaseStream.Seek(4, SeekOrigin.Current);
                    int matNum = matIndex;
        ...
        private int nonUVvtxLocIndexPMX = 0;
        ...
        private void ImportVertices()
        {
            ...
            //ushort vtxType, vtxTypeB;
            ulong vtxTypeLong = sp.ReadU32();
            ushort vtxType = ((ushort)(vtxTypeLong & 0xFFFF));
            /*if (sp.Endian == Endian.Big)
            {
                vtxTypeB = sp.ReadU16();
                vtxType = sp.ReadU16();
            }
            ...
            {                
                if(vtxType == 0x01)
                {
                    ...
                    else
                    {
                        ...
                        vtx.Position = new PMXVector3(sp.ReadSingle() * _totalScale, sp.ReadSingle() * _totalScale, sp.ReadSingle() * _totalScale);
                        vtx.Normals = new PMXVector3(sp.ReadHalfFloat(), sp.ReadHalfFloat(), sp.ReadHalfFloat());
                        sp.BaseStream.Seek(vtxSize - 18, SeekOrigin.Current);
                    }
                }

                if(vtxType == 0x02 && sp.Endian == Endian.Big)
                {
                    vtx = GetVertexOfIndex(ref nonUVvtxLocIndexPMX);
                    vtx.UV.U = sp.ReadHalfFloat();
                    vtx.UV.V = sp.ReadHalfFloat();
                }
